...
- layout_output_record_list にレコード名（例：OTF1-REC）が指定されている場合は、そのレコード名（= layout_output_record_list のキーの値）を直接使って書き込みステップを記述してください（「空でないため」などの理由説明は不要で、動作のみを記述すること）。

  ➤ この場合、メイン処理（関数）内で WRITE 関数を呼び出す前に、入力レコード（例：ITF1-REC）を layout_output_record_list に指定されたレコード名（例：OTF1-REC）に設定するステップを必ず明示してください。
  Example:
  × ITF1-RECをOTF1-RECとして出力ファイルに書き込む（関数実行）
  〇 ITF1-RECをOTF1-RECに設定する。OTF1-RECとして出力ファイルに書き込む（関数実行）

  ➤ また、「OTF1情報を書き込む（関数）」セクションにおいても、最初の入力レコードを layout_output_record_list に指定されたレコード名

- 一方、layout_output_record_list が空である場合は、最初の入力レコード（例：ITF1-REC）を OTF1 に書き込むものとし、その設定ステップも明示してください。 
